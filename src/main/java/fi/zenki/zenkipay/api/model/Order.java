...
public class Order {
  ...
  **/
  ...
  public String toUrlQueryString(String prefix) {
    ...
    // add `zenkiOrderId` to the URL query string
    if (getZenkiOrderId() != null) {
      try {
        joiner.add(String.format("%szenkiOrderId%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getZenkiOrderId()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    // add `paymentInfo` to the URL query string
    if (getPaymentInfo() != null) {
      joiner.add(getPaymentInfo().toUrlQueryString(prefix + "paymentInfo" + suffix));
    }

    // add `settlementInfo` to the URL query string
    if (getSettlementInfo() != null) {
      joiner.add(getSettlementInfo().toUrlQueryString(prefix + "settlementInfo" + suffix));
    }
    ...
    if (getClaim() != null) {
      joiner.add(getClaim().toUrlQueryString(prefix + "claim" + suffix));
    }
    ...
    // add `paymentSignature` to the URL query string
    if (getPaymentSignature() != null) {
      try {
        joiner.add(String.format("%spaymentSignature%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getPaymentSignature()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
